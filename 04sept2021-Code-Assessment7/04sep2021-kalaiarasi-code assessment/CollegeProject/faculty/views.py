...
def login_check(request):
    try:
        ...
        getfaculty = Faculty.objects.filter(username=getUsername, password=getPassword)
        faculty_serialiser = FacultySerializer(getfaculty, many=True)
        print(faculty_serialiser.data)
        if (faculty_serialiser.data):
            for i in faculty_serialiser.data:
                getId = i["id"]
                getName = i["name"]
...
def prof_view(request):
    ...
    try:
        ...
        getfaculty = Faculty.objects.get(id=getUid)
        faculty_serialiser = FacultySerializer(getfaculty)
         

        return render(request, 'profile.html',{"data":faculty_serialiser.data})    
    except:
        return HttpResponse("Something went wrong....") 
